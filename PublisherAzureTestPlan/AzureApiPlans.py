...
class AzureApiPlans:
    ...
    def add_attachement_to_testresult(self,run_id,test_result_id,output_dir,filename):
        ...
        response = requests.request("POST",self.azure_url_project+"/test/Runs/"+str(run_id)+"/Results/"+str(test_result_id)+"/attachments?api-version=6.0-preview.1", headers=self.headers, data=payload)
