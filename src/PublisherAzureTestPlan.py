    ...
    def publish_results(self) -> None:
        try:
            ...
            for suite in suitesresults.suites:
                testpoints = self.azure_api.get_tests_points(
                    suitesresults.get_testcase_ids(suite))
                testpoints = [testpoint for testpoint in testpoints if (testpoint['testPlan']['id'] == str(
                    suite['planId']) and testpoint['suite']['id'] == str(suite['id']))]
                if testpoints:
                    for testpoint in testpoints:
                        tc = suite['testcases'][str(
                    ...
                    raise Exception(
                        'There is no testpoints to pubblish outcomes')

                endtime = datetime.strptime(
                    suite['endtime'], "%Y%m%d %H:%M:%S.%f").isoformat()
                starttime = datetime.strptime(
                    suite['starttime'], "%Y%m%d %H:%M:%S.%f").isoformat()
                run = self.azure_api.create_run(
                    suite['planId'], suite['name']+' '+str(ENVIRONNEMENT).upper(), starttime, endtime, USERNAME)
                payload = FormatterPayloadAzure.format_testresults_payload(
                    testpoints)
    ...
    def publish_attachements_to_results(self, run_id) -> None:
        ...
        for result in test_resutls['value']:
            if result['outcome'] == "Failed":
                ...
                self.azure_api.add_attachement_to_testresult(
                    run_id, result['id'], self.output_dir, filename)
